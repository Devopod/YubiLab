...
User request: {prompt}"""
    ...
    # === FLUTTER-SPECIFIC POST-BUILD CLEANUP (aggressive) ===
    # Scan ENTIRE project directory and remove ALL non-Flutter files and folders
    if all_files and project['language'] == 'flutter':
        step_start = time.time()
        flutter_cleanup_files = []
        flutter_cleanup_dirs = []
        import shutil as _shutil

        # Whitelist: ONLY these top-level items are allowed in a Flutter project
        flutter_allowed_toplevel = {
            'lib', 'web', 'android', 'ios', 'linux', 'macos', 'windows', 'test',
            'assets', 'fonts', 'images', 'build', '.dart_tool', '.idea',
            'pubspec.yaml', 'pubspec.lock', 'analysis_options.yaml', '.metadata',
            '.gitignore', '.packages', 'README.md', 'CHANGELOG.md', 'LICENSE',
        }

        # Non-Flutter files that should NEVER exist in a Flutter project
        non_flutter_files = {
            'requirements.txt', 'run.py', 'main.py', 'app.py', 'manage.py',
            'Procfile', 'package.json', 'package-lock.json', 'yarn.lock',
            'Gemfile', 'Rakefile', 'Cargo.toml', 'go.mod', 'pom.xml',
            'index.php', 'composer.json', 'webpack.config.js', 'tsconfig.json',
            'server.js', 'index.js', '.babelrc', '.eslintrc', 'gulpfile.js',
        }

        # Non-Flutter directories that should NEVER exist in a Flutter project
        non_flutter_dirs = {
            'app', 'static', 'templates', 'node_modules', '__pycache__',
            'venv', 'env', '.venv', 'vendor', 'src', 'dist', 'public',
            'views', 'routes', 'controllers', 'migrations', 'config',
            'blueprints', 'api', 'services',
        }

        # Non-Flutter file extensions (excluding web/ directory)
        non_flutter_extensions = {
            '.py', '.rb', '.php', '.go', '.rs', '.java', '.kt',
            '.jsx', '.tsx', '.vue', '.svelte',
            '.css', '.scss', '.sass', '.less',
        }

        # Scan all top-level entries
        try:
            for entry in os.listdir(project_path):
                ...
                if os.path.isdir(full):
                    if entry in non_flutter_dirs or entry.startswith('__'):
                        _shutil.rmtree(full, ignore_errors=True)
                        flutter_cleanup_dirs.append(entry + '/')
                    # Also remove unknown top-level dirs that aren't in the whitelist
                    elif entry not in flutter_allowed_toplevel and not entry.startswith('.'):
                        # Check if it contains only non-Flutter files
                        has_dart = False
                        for _r, _d, _f in os.walk(full):
                            if any(f.endswith('.dart') for f in _f):
                                has_dart = True
                                break
                        if not has_dart:
                            _shutil.rmtree(full, ignore_errors=True)
                ...
                # Check top-level files
                    ...
                    if entry in non_flutter_files:
                        os.remove(full)
                        flutter_cleanup_files.append(entry)
                    elif ext in non_flutter_extensions:
                        os.remove(full)
                        flutter_cleanup_files.append(entry)

            # Also scan inside lib/ for non-Dart files
            lib_path = os.path.join(project_path, 'lib')
            if os.path.isdir(lib_path):
                for root, dirs, files in os.walk(lib_path):
                    for f in files:
                        _, ext = os.path.splitext(f)
                        if ext in non_flutter_extensions:
                            full_f = os.path.join(root, f)
                            os.remove(full_f)
                            rel = os.path.relpath(full_f, project_path)
                            flutter_cleanup_files.append(rel)

        except Exception:
            pass  # Don't fail the build if cleanup has issues

        total_cleaned = len(flutter_cleanup_files) + len(flutter_cleanup_dirs)
        if total_cleaned:
            items = flutter_cleanup_dirs + flutter_cleanup_files
            cleanup_names = ', '.join(items[:8])
            if total_cleaned > 8:
                cleanup_names += f' +{total_cleaned - 8} more'
            live_log.append({'icon': '\U0001f9f9', 'message': f'Auto-cleaned {total_cleaned} non-Flutter items: {cleanup_names}', 'type': 'fix'})
            add_step('Cleaning non-Flutter files', 'done', f'Removed {len(flutter_cleanup_files)} files + {len(flutter_cleanup_dirs)} dirs', time.time() - step_start)
